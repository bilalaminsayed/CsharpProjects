...
class LINQWithListCollection
{
   static void Main()
   {
      ...
      // convert to lowercase, select those that contain e and sort by reverse alphabetical order
      var lowerWithe =
         from item in items
         let lowercaseString = item.ToLower()
         where lowercaseString.Contains("e")
         orderby lowercaseString descending
         select lowercaseString;

      // display query results
      Console.Write("results of query lowerWithe:");
      foreach (var item in lowerWithe)
      {
         Console.Write($" {item}");
      }
